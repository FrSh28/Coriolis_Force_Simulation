...
disc = cylinder(frame = plate, pos = (0, -0.5, 0), radius = 7, color = color.gray(0.7), axis = (0, 0.5, 0), material = materials.rough)
...
def mouse_method(evt):
    ...
    if evt.click == "left":
        ...
        balls.append(sphere(pos = (plate.frame_to_world(player.pos).x, 0, plate.frame_to_world(player.pos).z), v = count_v(dt, poss) + balls_v * norm(plate.frame_to_world(throw.axis)),
                            radius = 0.3, color = color.red, material = materials.rough, make_trail = True, opacity = 0.5, tleft = 3.0, num = balln,
